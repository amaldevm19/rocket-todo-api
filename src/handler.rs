use crate::{
    ...
    response::{GenericResponse, TodoData, SingleTodoResponse, TodoListResponse},
};
...
pub async fn health_checker_handler()->Result<Json<GenericResponse>,Status>{
    const MESSAGE: &str = "Build simple CURD Api with Rust and Rocket";
...
pub async fn create_todo_handler(
    ...
    for todo in vec.iter(){
        if todo.title == body.title{
            let id = todo.id.to_owned().unwrap();
            let error_response = GenericResponse{
                status:"fail".to_string(),
                message:format!("Todo with title:'{}' already exisit", id)
            };
    ...
    vec.push(todo.into_inner());

    let json_response = SingleTodoResponse{
        status : "Success".to_string(),
        data : TodoData {
            todo: body.into_inner()
        }
    };
    Ok(Json(json_response))
...
pub async fn edit_todo_handler(
    ...
    for todo in vec.iter_mut(){
        if todo.id == Some(id.clone()){
            let datetime = Utc::now();
            let title = body.title.to_owned().unwrap();
            let content = body.content.to_owned().unwrap();
            let payload = Todo{
                id: todo.id.to_owned(),
...
pub async fn delete_todo_handler(
    ...
    let mut todos_vec = data.todo_db.lock().unwrap();

    for todo in todos_vec.iter_mut(){
        if todo.id == Some(id.clone()){
            todos_vec.retain(|todo|todo.id != Some(id.to_owned()));
